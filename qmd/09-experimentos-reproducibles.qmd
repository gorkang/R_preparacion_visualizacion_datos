...
```{r, eval=FALSE}
jsPsychMaker::create_protocol(canonical_tasks = c("AIM", "EAR", "IRI"), # Pruebas a incluir
                              folder_output = "~/Downloads/protocol999", # El directorio tiene que incluir un número (se usará como pid)
                              launch_browser = TRUE) # Abre el navegador con el protocolo
```
...
```{r, eval=FALSE}
  jsPsychMaker::create_protocol(canonical_tasks = "EAR", # Incluye la tarea EAR
                                folder_tasks = "~/Downloads/TEST/", # Crea e incluye las tareas que estan en esta carpeta
                                folder_output = "~/Downloads/TEST/new_protocol", # Crea el protocolo aquí
                                launch_browser = TRUE) # Lanza un navegador
```
