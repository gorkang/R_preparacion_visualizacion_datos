...
```{r facet_grid}
  ...
  ggplot(gapminder, aes(gdpPercap, lifeExp, color = country)) +
    geom_point(alpha = .2) +
    facet_grid(continent ~ pop > 5000000) + # pop > 5000000 es una condición lógica que será TRUE or FALSE
    guides(color = "none")
...
<details><summary>Solucion: ![](../data/images/hint.png)</summary><span style="color: orange;">`facet_grid()` permite crear rejillas simétricas de paneles, y el parámetro `nrow` de `facet_wrap()` nos ayuda con paneles con números de filas definidos.</span></details>
...
###### Ejemplos de funciones que podemos usar en los gráficos {.parameters -}
...
<details><summary>Pista: ![](../data/images/hint.png)</summary><span style="color: orange;">Tendrás que reemplazar `geom_line()` por `stat_summary()`, usando el parámetro `geom = "line"` para indicarle que quieres usar lineas en lugar de puntos.</span></details>
...
<details><summary>Pista: ![](../data/images/hint.png)</summary><span style="color: orange;">Tienes que encontrar los parámetros adecuados para `stat_summary()` y `facet_grid()`. Puedes ver ejemplos en: <BR> - [computaciones con ggplot: stat_summary()](02-visualizacion-avanzada.html#computaciones-con-ggplot-stat_summary)  <BR> - [Facet_grid](02-visualizacion-avanzada.html#facet_grid).</span></details>
...
```{r personalizacion-graficas-axis}
  ...
  plot_base + 
    scale_y_continuous(n.breaks = 15) +
    scale_x_continuous(n.breaks = 6, labels = scales::dollar_format(prefix = "$", suffix = "M"))
  
  # Escala log
  plot_base + 
    scale_y_continuous(n.breaks = 15) +
    scale_x_log10(n.breaks = 4, labels = scales::dollar_format(prefix = "$", suffix = "M"))
...
<details><summary>Soluciones: ![](../data/images/hint.png)</summary><span style="color: orange;">- Recuerda la función`scales::comma()` que vimos más arriba <BR>- `theme(legend.position = "ALGO AQUI")` nos permite mover la leyenda. Si vas la ayuda de `theme()` y buscas `legend.position`, encontrarás sus opciones. <BR>- Los parámetros de `labs()` nos permiten añadir títulos, subtítulos, editar los valores de las etiquetas de `x` e `y`, añadir `caption`, etc. </span></details>
...
<details><summary>Soluciones: ![](../data/images/hint.png)</summary><span style="color: orange;">Hemos visto como hacer esto en el primer ejemplo de [fancy pants legends](02-visualizacion-avanzada.html#fancy-pants-legends) </span></details>
...
plot1 = plotly::plot_ly(x = ~ colnames(DF),
  ...
  plotly::add_surface(name = "3D mesh",
                      connectgaps = TRUE, hidesurface = TRUE, showscale = FALSE,
                      contours = list(
