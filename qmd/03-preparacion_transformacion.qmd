...
Aquí nos quedamos con las filas donde el valor de Educación sea mayor de 8 **Y** el Genero sea igual a 1 (fijate que usamos `==` para indicar la condición lógica de igualdad):  
...
Si queremos ordenar a partir de dos o más criterios, simplemente los separamos por coma. Además, si queremos que alguno de ellos sea de manera descentente (por defecto es ascendente), envolvemos el nombre de columna en `desc()`. 
...
1. Usando la base df_wide (ver código abajo), haz las siguientes cosas, una a una:  
...
* Crea una nueva variable llamada `niv_edu_porc`, en la que calcules el nivel educativo al que han llegado dividido por el máximo de la base de datos, pero en porcentaje (nivel educativo persona / nivel educativo maximo; en porcentaje)  
