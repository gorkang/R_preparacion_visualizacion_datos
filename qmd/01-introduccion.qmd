...
Como indico en la [introducción](/#sobre-el-uso-de-llms-chatgpt-gemini-and-friends-llms), recomiendo **no usar** LLMs, o hacerlo como último recurso. Si despues de intentar resolver un ejercicio un buen rato, googlear, buscar en Stack Overflow, consultarlo con las compañeras/os, ... seguimos sin conseguirlo, podemos usar LLMs para que nos de la solución. Pero esto será útil, únicamente si después dedicamos el esfuerzo necesario a entender ese código, y ser capaces de reproducirlo nosotros (lo mismo ocurre con soluciones que encontremos en Stack Overflow, etc.).  
