...
Si partimos de un nuevo proyecto de RStudio llamado `mi_primer_proyecto` en el que hemos añadido un reporitorio de Git (`usethis::use_git()`), y creado un nuevo archivo llamado `mi_primer_archivo.R`, en el panel `Git` veremos algo similar a esto:  
...
Por ejemplo, si añadimos una línea, veremos que el ícono junto al archivo se convierte en una M en un recuadro azul. 
...
2. Haz un commit de ese archivo y subelo (push) a Github (asegurate que esta allá!). No olvides hacer un pull!  
