...
# table_combined_vive = gtsummary::tbl_merge(list(T_female, T_male), tab_spanner = list("Female", "Male"))

# gt::gtsave(gtsummary::as_gt(table_combined_vive), file = "outputs/table-combined-vive.png")
...
# https://github.com/easystats/easystats
  ...
  report(corr_test)
  table_easystats = report(corr_test) %>% table_long()
...
# https://paulvanderlaken.com/2018/09/10/simpler-correlation-analysis-in-r-using-tidyverse-principles/
  table_correlations <- iris %>% 
    correlation()  
    # report() %>% 
    # table_long(full = TRUE)
  
# Cambiamos todos los NA por ""
table_clean = table_correlations %>% 
...
```{r tablas-bonitas-lm}
  ...
  report(model_lm, standardize = "robust") %>% text_short()
  table_easystats = report(model_lm, standardize = "robust") %>% table_long()
  knitr::kable(table_easystats)
