...
```{r visualizacion-importante-01b}
# Modelo de regresion
...
```{r visualizacion-importante-01c}
...
```{r visualizacion-importante-01d}
...
```{r visualizacion-importante-05, fig.width=12}
...
**Antes** de empezar, asegúrate que tienes instalados los paquetes `tidyverse` y `gapminder`, y crea el DF `pop_simple` usando el código de abajo.  
...
<details><summary>Solucion: ![](data/images/hint.png)</summary><span style="color: orange;">color = continent debe ir dentro de aes()<BR>color = "blue" debe ir fuera de aes()</span></details>
...
```{r transformaciones-estadisticas-proporciones}
...
```{r transformaciones-estadisticas-stat_summary}
...
```{r transformaciones-estadisticas-promedios-grupo}
...
ggplot(mpg) + 
  geom_jitter(aes(x = class, y = hwy), width = 0.2) +
  stat_summary(aes(x = class, y = hwy), fun = mean, color = "red", geom = "point", size = 4, alpha = .7) +
  stat_summary(aes(x = class, y = hwy), fun = median, color = "green", geom = "point", shape = "triangle", size = 4, alpha = .7) + 
  theme_minimal()
```
