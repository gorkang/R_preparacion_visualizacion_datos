...
En ggplot, después de indicar los datos y coordenadas (e.g. `ggplot(mpg, aes(displ, hwy))`), podemos sumar uno o varios `geoms` con una lógica de capas superpuestas, por ejemplo, `+ geom_point()`.   
...
Podemos usar líneas de tendencia con `geom_smooth()`. El `method` por defecto es [loess](https://en.wikipedia.org/wiki/Local_regression), pero podemos usar otros distintas funciones (e.g. `geom_smooth(method = "lm")` para usar una regresión lineal).  
...
<details><summary>Solución: ![](data/images/hint.png)</summary><span style="color: orange;">Tienes que usar el `geom_bar()` y el parámetro `fill`.</span></details>
