...
* Webs como [R bloggers](www.r-bloggers.co)  
...
```{r color, message=FALSE, warning=FALSE, cache=TRUE}
  ...
  ggplot(gapminder, aes(gdpPercap, lifeExp, color = "red")) + 
    geom_point()
...
```{r color-01, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  ggplot(gapminder, aes(gdpPercap, lifeExp, color = c("red", "grey", "green", "purple", "black"))) + 
    geom_point() 
...
```{r ejercicio-7a}
plotA = ggplot(data = mpg,
       mapping = aes(x = displ, y = hwy, color = class)) +
  geom_point() +
  theme(legend.position = "bottom")

plotB = ggplot(data = mpg,
       mapping = aes(x = displ, y = hwy, color = class)) +
  geom_point() +
  geom_line() +
  theme(legend.position = "bottom")
...
```{r ejercicio-8, echo=FALSE}
  ...
  ggplot(gapminder) + 
    stat_summary(
      aes(country, lifeExp, color = continent),
