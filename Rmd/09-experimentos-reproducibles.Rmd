...
2)  Abrimos `run.R` y seguimos las instrucciones. En esencia:
    ...
    A) Averiguamos cual es nuestro número de protocolo:   
    
      - Si lo desconoces, es el primer número de los archivos de datos, antes del "_". Por ejemplo, **134**_Consent_original_2022-04-02T205622_18840434.csv -> número de protocolo 134  
      
    B) Ponemos los datos (archivos csv) en la carpeta `data/ID_PROTOCOLO` (**reemplazar ID_PROTOCOLO por el número de nuestro protocolo**.  
    
      + Si la carpeta `data/ID_PROTOCOLO` contiene algun archivo, borrar antes de copiar nuestros datos: Por ejemplo, si usas el 999, BORRA el archivo 999.zip que hay en `data/999` antes de copiar tus datos.  

    C) Correr las líneas de abajo (**el parametro `pid` en `run_initial_setup()` tiene que tener tu número de protocolo**. Por ejemplo, si mi protocolo es el 134, `pid = 134`)  

```{r, eval=FALSE}

# Setup de nuestro sistema
